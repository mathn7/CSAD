...
X1 = 3*rand(nb_indiv1,nb_param);X1 = X1 - 0.5*ones(nb_indiv1,1)*ones(1,nb_param);  
% Creation de la premiere classe autour de l'element moyen + (1 .... 1)
X2 = 3*rand(nb_indiv2,nb_param);X2 = X2 + 1*ones(nb_indiv2,1)*ones(1,nb_param); 
% Creation du tableau des donnees (concatenation des deux classes) 
% et du tableau centré des donnees
X = [X1;X2]; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
x_mean = mean(X,1); Xc = X-ones(nb_indiv,1)*x_mean; 
S = 1/nb_indiv*(Xc'*Xc); 
[W,D] = eig(S); 
[D,tri]=sort(diag(D),'descend');
W = W(:,tri);
C = Xc*W;
...
hold on,plot(X(1:nb_indiv1,1),zeros(nb_indiv1,1),'r*'); 
% ceux de la seconde classe sont en bleu
hold on,plot(X(nb_indiv1+1:nb_indiv,1),zeros(nb_indiv2,1),'b*');
title('Visualisation des donnees sur les deux premiers axes canoniques')

% Affichage des donnees sur les premieres composantes principales : (même
% code couleur)
subplot(2,1,2)
plot([min(C(:,1))-1 max(C(:,1))+1],[0 0],'k-')
hold on,plot(C(1:nb_indiv1,1),zeros(nb_indiv1,1),'r*');
hold on, plot(C(nb_indiv1+1:nb_indiv,1),zeros(nb_indiv2,1),'b*');
...
hold on,plot(C(n1+1:n1+n2,1),zeros(1,n2),'b*')
hold on,plot(C(n1+n2+1:n1+n2+n3,1),zeros(1,n3),'g*')
hold on,plot(C(n1+n2+n3+1:n1+n2+n3+n4,1),zeros(1,n4),'m*');
title('1ere composante ppale')

subplot(3,1,2)
plot([min(C(:,2))-0.5 max(C(:,2))+0.5],[0 0],'k-'), 
hold on,plot(C(1:n1,2),zeros(1,n1),'r*'); 
hold on,plot(C(n1+1:n1+n2,2),zeros(1,n2),'b*')
hold on,plot(C(n1+n2+1:n1+n2+n3,2),zeros(1,n3),'g*')
hold on,plot(C(n1+n2+n3+1:n1+n2+n3+n4,2),zeros(1,n4),'m*');
title('2eme composante ppale')

subplot(3,1,3)
plot([min(C(:,3))-0.5 max(C(:,3))+0.5],[0 0],'k-'), 
hold on,plot(C(1:n1,3),zeros(1,n1),'r*'); 
hold on,plot(C(n1+1:n1+n2,3),zeros(1,n2),'b*')
hold on,plot(C(n1+n2+1:n1+n2+n3,3),zeros(1,n3),'g*')
hold on,plot(C(n1+n2+n3+1:n1+n2+n3+n4,3),zeros(1,n4),'m*');
...
hold on,plot(C(n1+1:n1+n2,1),C(n1+1:n1+n2,2),'b*')
hold on,plot(C(n1+n2+1:n1+n2+n3,1),C(n1+n2+1:n1+n2+n3,2),'g*')
hold on,plot(C(n1+n2+n3+1:n1+n2+n3+n4,1),C(n1+n2+n3+1:n1+n2+n3+n4,2),'m*');
grid on 
...
hold on, plot3(C(n1+1:n1+n2,1),C(n1+1:n1+n2,2),C(n1+1:n1+n2,3),'b*');
hold on, plot3(C(n1+n2+1:n1+n2+n3,1),C(n1+n2+1:n1+n2+n3,2),C(n1+n2+1:n1+n2+n3,3),'g*');
hold on,plot3(C(n1+n2+n3+1:n1+n2+n3+n4,1),C(n1+n2+n3+1:n1+n2+n3+n4,2),C(n1+n2+n3+1:n1+n2+n3+n4,3),'m*');
grid on
legend('ind. de la classe 1','ind. de la classe 2','ind. de la classe 3','ind. de la classe 4')
